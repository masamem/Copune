...
export function CouponModal() {
 ...
 const goStore = () => {
  ...
  store.url;
  const finalUrl =
  active.affiliate_url || store.affiliate_url
    ? destinationUrl
  ...
  className="font-display text-lg font-extrabold text-ink-950"
>
  {active.code ? (
    <>
      تم نسخ الكود بنجاح{" "}
      <PartyPopper className="inline h-5 w-5 text-gold-500" />
    </>
  ) : (
    "عرض متاح الآن"
  )}
...
{active.code && (
  <div className="mt-6 rounded-xl border-2 border-dashed border-brand-300 bg-brand-50/50 p-5 text-center">
    ...
    <div className="mt-2 flex items-center justify-center gap-3">
      <span
        className="select-all font-mono text-3xl font-bold tracking-[0.18em] text-brand-950"
        dir="ltr"
      >
        {active.code}
      </span>
  ...
  {active.code
    ...
    : "سنفتح العرض مباشرة على موقع المتجر في نافذة جديدة."}
</p>
        ...
        <div className="mt-6 rounded-xl bg-ink-50/70 p-4">
          {!myVote ? (
            <>
              <p className="text-center text-sm font-bold text-ink-800"> {active.code ? "هل عمل الكود معك؟" : "هل كان العرض متاحاً؟"}</p>
        ...
        <Link to={`/offer/${active.id}`} onClick={closeModal} className="mt-5 block text-center text-xs font-bold text-brand-700 underline-offset-4 hover:underline">
          {active.code ? "عرض تفاصيل الكوبون كاملة" : "عرض تفاصيل العرض كاملة"}
        </Link>
      </div>
    </Modal>
  );
